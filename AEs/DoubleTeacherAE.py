...
def double_teacher_train(device=None, epochs=10, save=True, scheduler_type=None, scheduler_kwargs=None, combine_method='mean'):
	...
	for epoch in range(student_epochs):
		for (imgs1, _), (imgs2, _) in zip(train_loader1, train_loader2):
			...
			# AE3: 教師2つの出力（concat）のみで学習
			with torch.no_grad():
				z1 = ae1.enc(imgs1)
				z2 = ae2.enc(imgs2)
				z_student = torch.cat([z1, z2], dim=1)
				x_student_input = z_student
				x1_hat = ae1(imgs1)
				x2_hat = ae2(imgs2)
				x_teacher_target = (x1_hat + x2_hat) / 2
			opt3.zero_grad()
			x3_hat = ae3.dec(x_student_input).view(-1, 1, 28, 28)
			loss3 = loss_fn(x3_hat, x_teacher_target)
			loss3.backward()
			opt3.step()
	...
	if save:
		torch.save(ae1.state_dict(), os.path.join(os.path.dirname(__file__), "pths", "teacher1_ae.pth"))
		torch.save(ae2.state_dict(), os.path.join(os.path.dirname(__file__), "pths", "teacher2_ae.pth"))
		torch.save(ae3.state_dict(), os.path.join(os.path.dirname(__file__), "pths", "student_ae.pth"))
	return ae1, ae2, ae3
...
def double_teacher_test(ae1, ae2, ae3, device=None, test_loader=None, save_fig=False):
	...
	with torch.no_grad():
		...
		recon_ae1 = ae1(imgs)
		recon_ae2 = ae2(imgs)
		recon_ae3 = ae3.dec(z_student).view(-1, 1, 28, 28)
