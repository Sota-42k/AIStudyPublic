	...
	for epoch in range(epochs):
		...
		with torch.no_grad():
			...
			for digit in range(10):
				...
				save_image(grid, os.path.join(save_dir, f"epoch_{epoch}_digit_{digit}.png"))
			# Optionally, save a grid of all digits
			all_samples = torch.cat(all_samples, dim=0)
			grid = make_grid(all_samples, nrow=8)
			save_image(grid, os.path.join(save_dir, f"epoch_{epoch}_all_digits.png"))
	torch.save(model.state_dict(), os.path.join(save_dir, "ddpm_mnist_cond.pth"))
